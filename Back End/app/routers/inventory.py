...
async def create_inventory(inventory: inventory.InventoryBase, repository: InventoryRepo = Depends(current_inventory_repo)):
    inventory_posted = repository.create_inventory(inventory)
    return inventory_posted

#Get all Inventorys with the new sql wrapper
@router.get("/", response_model=List[inventory.InventoryModel], status_code=status.HTTP_200_OK)
...
async def update_inventory(item_id: int, inventory_update: inventory.UpdateInventoryBase, repository: InventoryRepo = Depends(current_inventory_repo)):
